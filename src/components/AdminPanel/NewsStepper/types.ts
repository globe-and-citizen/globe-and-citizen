...
// Rules Analysis: max 650 words, 3000 characters
const rulesAnalysisSchema = z
  .string()
  .refine((v) => getTextLength(v) > 0, "Rules Analysis is required")
  .refine(
    (v) => getTextLength(v) <= 3000,
    "Rules Analysis must be at most 3000 characters",
  )
  .refine(
    (v) => getWordCount(v) <= 650,
    "Rules Analysis must be at most 650 words",
  );
